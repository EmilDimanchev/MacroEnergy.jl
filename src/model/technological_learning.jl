function add_learning!(system::System, model::Model, period_idx::Int, settings::NamedTuple)
    ```
    Implements endogenous technological learning constraints. The main output is the endogenous investment cost, called "annualized_investment_cost_with_learning", which is used in edge.jl for any learning technologies
    ...
    for learning_tech in learning_techs
        ...
        for e in learning_tech_edges
            ...
            ϵ = ones(length(x_points))*epsilon_learning # ensures the first inequality is strict
            ...
            if curr_period <= cc_duration(e)
                ...
                if settings[:ProjectDevelopment]
                    ...
                    # Big M constraints
                    @constraint(model, [k in 1:n_segments+1], e.new_de_capacity - e.aux_new_capacity_de[k] <= max_new_capacity(e)*(1-segments_sos1_prev(e)[k]))
                    @constraint(model, [k in 1:n_segments+1], e.aux_new_capacity_de[k] <= max_new_capacity(e)*e.segments_sos1_prev[k])
                    ...
                    # Big M constraints
                    @constraint(model, [k in 1:n_segments+1], e.new_af_capacity - e.aux_new_capacity_af[k] <= max_new_capacity(e)*(1-segments_sos1_prev(e)[k]))
                    @constraint(model, [k in 1:n_segments+1], e.aux_new_capacity_af[k] <= max_new_capacity(e)*e.segments_sos1_prev[k])
                    ...
                    # Big M constraints
                    @constraint(model, [k in 1:n_segments+1], e.new_cc_capacity - e.aux_new_capacity_cc[k] <= max_new_capacity(e)*(1-segments_sos1_prev(e)[k]))
                    @constraint(model, [k in 1:n_segments+1], e.aux_new_capacity_cc[k] <= max_new_capacity(e)*e.segments_sos1_prev[k])
