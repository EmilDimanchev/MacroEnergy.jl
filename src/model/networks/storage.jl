...
function compute_investment_costs!(g::AbstractStorage, model::Model, cost_type::Function=pv_period_investment_cost; settings::NamedTuple=NamedTuple())
    if has_capacity(g)
        if can_expand(g)
...
function compute_fixed_costs!(g::AbstractStorage, model::Model, cost_type::Symbol=:PV; settings::NamedTuple=NamedTuple())
    ...
    compute_investment_costs!(g, model, invesment_cost_function[cost_type]; settings=settings)
