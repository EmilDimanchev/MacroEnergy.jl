...
"""
function add_model_constraint!(ct::MinFlowConstraint, e::UnidirectionalEdge, model::Model, settings::NamedTuple)
    if has_capacity(e)
...
function add_model_constraint!(ct::MinFlowConstraint, e::BidirectionalEdge, model::Model, settings::NamedTuple=NamedTuple())
...
!!! note
    This constraint is available only for unidirectional edges.
"""
function add_model_constraint!(ct::MinFlowConstraint, e::EdgeWithUC, model::Model, settings::NamedTuple)
    ct.constraint_ref = @constraint(
        model,
        [t in time_interval(e)],
